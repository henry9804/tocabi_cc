...
namespace QP
{
    ...
    void CartesianVelocity::setCurrentState(const Eigen::Matrix<double, JOINT_DOF, 1> &q_current, const Eigen::Matrix<double, JOINT_DOF, 1> &qdot_current, const Eigen::Matrix<double, TASK_DOF, JOINT_DOF> &j_current)
    {
    ...
    void CartesianVelocity::setDesiredEEVel(const Eigen::Matrix<double, TASK_DOF, 1> &xdot_desired)
    {
    ...
    void CartesianVelocity::setWeightMatrix()
    {
        ...
        qddot_weight_(0,0) = qddot_weight[0];
        qddot_weight_(1,1) = qddot_weight[1];
        qddot_weight_(2,2) = qddot_weight[2];
        qddot_weight_(3,3) = qddot_weight[3];
        qddot_weight_(4,4) = qddot_weight[4];
        qddot_weight_(5,5) = qddot_weight[5];
        qddot_weight_(6,6) = qddot_weight[6];
        qddot_weight_(7,7) = qddot_weight[7];
    ...
    {
        P_.block(si_index_.s1,  si_index_.s1,  ns_, ns_) = 2.0 * slack_weight_;
        P_.block(si_index_.dq1, si_index_.dq1, nq_, nq_) = 2.0 * damping_weight_;
        P_.block(si_index_.dq1, si_index_.dq1, nq_, nq_) += 2.0 * qddot_weight_;
        ...
        q_.block(si_index_.dq1, 0, nq_, 1) = -2. * qddot_weight_ * qdot_current_;
    }

    void CartesianVelocity::setConstraint()
    {
        A_.block(si_index_.con_slack, si_index_.s1,  ns_, ns_).setIdentity(ns_, ns_);
        A_.block(si_index_.con_slack, si_index_.dq1, ns_, nq_) = j_current_;
        A_.block(si_index_.con_q,     si_index_.dq1, nq_, nq_).setIdentity(nq_, nq_);
        A_.block(si_index_.con_qdot,  si_index_.dq1, nq_, nq_).setIdentity(nq_, nq_);
        A_.block(si_index_.con_qddot, si_index_.dq1, nq_, nq_).setIdentity(nq_, nq_);
    ...
    bool CartesianVelocity::getOptJointVel(Eigen::Matrix<double, JOINT_DOF, 1> &opt_qdot)
    {
        setCost();
        setConstraint();
        ...
        {
            opt_qdot = sol.segment(si_index_.dq1, JOINT_DOF);
            return true;
        }
        else
        {

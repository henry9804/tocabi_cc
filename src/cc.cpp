    ...
    else if (rd_.tc_.mode == 8)
    {
        ...
        if(data_collect_start_){
            ...
            if(make_dir)
            {
                ...
                string folderPath3 = "/home/lyh/catkin_ws/src/tocabi/tocabi_srmt/data";

                int result = mkdir(folderPath.c_str(), 0777);
                int result2 = mkdir(folderPath2.c_str(), 0777);
                int result3 = mkdir(folderPath3.c_str(), 0777);
                if(result != 0){
                    ROS_ERROR("Couldn't make folder(dir), Check folderPath");
                }
                if(result2 != 0){
                ...
                if(result3 != 0){
                    ROS_ERROR("Couldn't make folder3(dir), Check folderPath3");
                }
                make_dir = false;
                std::stringstream fileNameSS2;
                fileNameSS2 << "text-" << ros::Time::now().sec << "-" << ros::Time::now().nsec << + ".txt";
                ...
                std::stringstream fileNameSS3;
                fileNameSS3 << "traj-" << ros::Time::now().sec << + ".txt";
                string fileName3 = fileNameSS3.str();
                std::stringstream filePathSS3;
                filePathSS3 << folderPath3 << "/" << fileName3;
                string filePath3 = filePathSS3.str();
                fout2.open(filePath3);
                if(!fout2.is_open())
                {
            ...
            // float cur_time = now.sec + now.nsec * 10e-9;
            fout2 << (now - init).toSec() << " ";
            for(int i = 0; i < MODEL_DOF; i++)
            {
                fout2 << rd_.q_[i] << " ";
            }
